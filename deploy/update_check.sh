...
if [[ -d "${BASE_DIR}" ]]; then
  ...
  if [[ "${VERSION_CURRENT}" != "${VERSION_NEW}" ]]; then
    printf "\r\033[0;93m[UPDATE]\033[0;90m Online update\033[0m"
    ...
    if [[ ${EXIT_CODE} -eq 0 ]]; then
      ...
      if [[ ${EXIT_CODE} -ne 0 ]]; then
        printf "\r\033[0;91m[FAILED]\033[0;90m Online update\033[0m"
        printf "\n\033[0;94m[SCRIPT]\033[3;94m %s\033[0m" "${COMMAND}"
        printf "\n%s" "${OUTPUT[@]}"
      else
        printf "\r\033[0;92m[  OK  ]\033[0;90m Online update\033[0m"
